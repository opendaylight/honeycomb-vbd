...
import org.opendaylight.yang.gen.v1.urn.opendaylight.params.xml.ns.yang.vbridge.topology.rev160129.network.topology.topology.node.termination.point._interface.type.VirtualDomainCarrierCaseBuilder;
import org.opendaylight.yang.gen.v1.urn.opendaylight.params.xml.ns.yang.vbridge.topology.rev160129.network.topology.topology.node.termination.point._interface.type.virtual.domain.carrier._case.VirtualDomainCarrierBuilder;
...
public final class VirtualBridgeDomainManager implements AutoCloseable {
        ...
        Executors.newSingleThreadExecutor().submit(() -> {
            for (int i = 0; i < 3; i++) {
                ...
                try {
                    ...
                    LOG.warn("Failed to write startup data to operational DS. Retrying ...");
                    try {
                        wTx.cancel();
                    } finally {
                        wTx = null;
    ...
    public static void writeStartupConfig(@Nonnull DataBroker dataBroker, String virtualDomainInterfaces) {
        if (Strings.isNullOrEmpty(virtualDomainInterfaces)) {
            return;
        }
        LOG.warn("VIRTUAL DOMAIN INTF INSTANTIATE TX" + virtualDomainInterfaces);
        Topology confTopology = new TopologyBuilder().setTopologyId(VbdUtil.STARTUP_CONFIG_TOPOLOGY).addAugmentation(TopologyVbridgeCfgAugment.class,
                new TopologyVbridgeCfgAugmentBuilder().setVbridgeStartupConfig(new VbridgeStartupConfigBuilder().build()).build()).build();
        TerminationPointVbridgeCfgAugmentBuilder tpAug = new TerminationPointVbridgeCfgAugmentBuilder()
                .setInterfaceType(new VirtualDomainCarrierCaseBuilder().setVirtualDomainCarrier(new VirtualDomainCarrierBuilder().build()).build());
