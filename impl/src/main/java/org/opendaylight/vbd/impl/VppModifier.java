...
final class VppModifier {
    ...
    VppModifier(final MountPointService mountService, final String bridgeDomainName,
        ...
        this.iiBridgeDomainOnVPP = InstanceIdentifier.create(BridgeDomains.class)
    ...
    private ListenableFuture<Optional<Ipv4AddressNoZone>> readIpAddressFromVpp(@Nonnull DataBroker localDataBroker,
        ...
        final DataBroker vppDataBroker = VbdUtil.resolveDataBrokerForMountPoint(iiToVpp, mountService);
        if (vppDataBroker == null) {
            LOG.warn("Data broker for vpp {} not found.", iiToVpp);
            resultFuture.set(Optional.absent());
            return resultFuture;
        }
        final Optional<Interfaces> optInterfaces =
        ...
        for (Interface intf : optInterfaces.get().getInterface()) {
         ...
         */
        for (Interface intf : optInterfaces.get().getInterface()) {
            if (intf.getType().equals(EthernetCsmacd.class) && TENANT_INTERFACE.equals(intf.getDescription())) {
                final Optional<Ipv4AddressNoZone> ipOp = readIpAddressFromInterface(intf, iiToVpp);
                if (ipOp.isPresent()) {
                    resultFuture.set(ipOp);
                    tenantInterfaceIpCache.put(iiToVpp, ipOp);
            ...
            for (final Interface bdInterface : interfaceList) {
                ...
                if (subInterfaceAugmentation != null) {
                    ...
                    for (SubInterface subInterface : subInterfaces.getSubInterface()) {
                        final org.opendaylight.yang.gen.v1.urn.opendaylight.params.xml.ns.yang.vpp.vlan.rev170607.sub._interface.l2.config.attributes.L2
                            l2 = subInterface.getL2();
                            ...
                            if (interconnection instanceof BridgeBased) {
                                ...
                                if (bridgeDomainName.equals(bridgeBased.getBridgeDomain())) {
                                    LOG.trace("Sub-interface {} has assigned bd which will be removed. Disconnecting ...",
                                            bdInterface.getName());
                                    final InstanceIdentifier<org.opendaylight.yang.gen.v1.urn.opendaylight.params.xml.ns.yang.vpp.vlan.rev170607.sub._interface.l2.config.attributes.L2> augmentationIid = InstanceIdentifier.create(Interfaces.class)
                                            ...
                                            .child(org.opendaylight.yang.gen.v1.urn.opendaylight.params.xml.ns.yang.vpp.vlan.rev170607.sub._interface.l2.config.attributes.L2.class).builder().build();
                                    VbdNetconfTransaction.netconfSyncedDelete(vppIid, augmentationIid,
                                        VbdNetconfTransaction.RETRY_COUNT);
